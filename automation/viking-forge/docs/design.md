...
4. 分诊任务使用独立 worktree 和 `codex exec --ephemeral --sandbox read-only`。
...
8. 修复任务使用新的 worktree 和新的 `codex exec --ephemeral --sandbox workspace-write`。
...
进程异常退出后，启动时把遗留 `running` 任务恢复为 `queued`。同一 Issue 最多有一个活动任务。Codex、Git、门禁、测试或发布失败时，Issue 进入 `blocked`，错误写入面板和飞书 Outbox。
