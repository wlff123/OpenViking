...
VikingForge 直接运行在持有本地仓库和 Codex 登录状态的受信任 Linux 用户下。当前环境使用 `openeuler00` 容器中的 `wlf1` 用户、`/data/wlf1/viking-forge-workspace` 仓库和 `/home/wlf1/.codex` 登录目录。服务只需一个 Python 进程；公网入口仅转发 GitHub Webhook，面板可以只监听 PC 本地端口。
