...
3. Worker 创建只读分诊 worktree，写入 Issue 上下文，执行独立的临时 Codex 会话；权限配置默认拒绝宿主文件，仅允许读取该 worktree。
...
6. Worker 创建新的修复 worktree并执行独立的临时 Codex 会话。权限配置仅允许写入该 worktree；Codex 不持有 GitHub App、Webhook 或飞书密钥。
...
- Codex 模型命令默认拒绝宿主文件系统，只开放 worktree、仓库 Git 元数据、验证虚拟环境和 Codex 临时目录；同时忽略用户配置与规则，并关闭应用、Hook、多 Agent、远程插件和联网能力。
...
Codex、Git、门禁或测试失败时，Worker 记录截断后的错误，移除生命周期标签，添加 `agent:blocked`，更新面板并写入飞书 Outbox。进程重启后，遗留的 `running` 任务会标记为 `failed`，对应 Issue 进入 `blocked`，等待维护者重新触发；同一 Issue 同时最多有一个活动任务。
